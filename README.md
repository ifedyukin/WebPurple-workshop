...
## Прежде, чем начать
> :exclamation: Если вы выполняете эти задания на компьютере, расположенно в аудитории, то этот пункт пропускаем.
...
* `required: true` - значение этого поля должно быть уникальным.
